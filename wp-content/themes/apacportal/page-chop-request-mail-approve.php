...
if(isset($_POST['submit'])){
	try{
		...
		add_post_meta($application_id, 'request_status', 'pending', true);
		
		add_post_meta($application_id, 'requestor_email', $requestor_email);
		
		if(empty($_POST['is_on_exempt_list'])){
			add_post_meta($application_id, 'approver', $_POST['approver']);
			add_post_meta($application_id, 'approver_email', $approver_email);
		}
		...
		if(empty($_POST['is_on_exempt_list'])){
			...
			add_post_meta($application_id, 'approve_hash', $approve_hash);
		...
		$success = 'Your request NO. is ' . $request_no . '. We have sent an email to ' . $approver_email;
				...
				<div class="control-group-area">
					...
					<div class="control-group">
						...
						<div class="controls">
							...
							<input type="text" id="requestor" name="requestor" value="<?=$_POST['requestor']?>" autocomplete="off" style="width:315px" placeholder="Name <prefix@fcagroup.com>" />
					...
					<div class="control-group approver-section">
						...
						<div class="controls">
							<input type="text" id="approver" name="approver" value="<?=$_POST['approver']?>" autocomplete="off" style="width:315px" placeholder="Name <prefix@fcagroup.com>" />
